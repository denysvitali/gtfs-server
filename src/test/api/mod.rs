...
use importer::serde::de::{Deserialize, Deserializer, MapAccess, Visitor};
...
fn create_server(routes: Vec<Route>) -> Rocket {
    ...
    rocket::ignite().manage(rh).mount("/api", routes)
}
...
pub fn stop_by_id() {
    ...
    assert_eq!(response.status(), Status::Ok);
    assert_eq!(response.content_type(), Some(ContentType::JSON));
    let res: Result<Stop> = serde_json::from_str(&response.body_string().unwrap()).unwrap();
...
pub fn invalid_stop_by_id() {
    ...
    assert_eq!(
        ...
        "Server responded w/ success:true when an invalid stop was provided"
    );
